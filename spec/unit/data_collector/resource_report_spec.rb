...
describe Chef::DataCollector::ResourceReport do
  ...
  let(:resource) { Chef::Resource.new("bar", run_context) }
  ...
  describe "#elapsed_time_in_milliseconds" do
    ...
    context "when elapsed_time is not set" do
      it "should return a number" do
        ...
        expect(report.elapsed_time_in_milliseconds).to be_kind_of Fixnum
      end
    end
  end
  ...
  describe "#to_hash" do
    context "for a simple_resource" do
      let(:resource) do
        z = Class.new(Chef::Resource) do
          resource_name "bar"
        end
        z.new("foo", run_context)
      end
      let(:hash) do
        {
          "after" => {},
    ...
    context "for a lazy_resource that got skipped" do
      let(:resource) do
        ...
        l = c.new("foo")
        l.sword = Chef::DelayedEvaluator.new { nil }
        l
      end
      let(:hash) do
        {
          ...
          "duration" => "",
          "conditional" => "because",
          "id" => "unknown identity (due to failed validation)",
          "ignore_failure" => false,
          "name" => "foo",
          "result" => "create",
          "status" => "skipped",
      ...
      it "should handle the ValidationFailed Exception and throw a dummy identity" do
        report.skipped(conditional)
        expect(report.to_hash).to eq hash
      end
    end
